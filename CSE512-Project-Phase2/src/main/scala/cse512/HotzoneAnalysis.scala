...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // Fetch the count of points in each rectangle, which gives a measure of the hotness of the rectangle
    val resultDf = spark.sql("select rectangle, COUNT(point) as pointCount from joinResult GROUP BY rectangle ORDER BY rectangle ASC")
    resultDf.createOrReplaceTempView("countResult")
